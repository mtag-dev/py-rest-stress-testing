...
from schema_dataclasses import UserInfoResponse, SprintResponse
...
@app.get("/api/v1/userinfo/{dynamic}", response_model=UserInfoResponse)
async def userinfo(dynamic: int):
    async with pool as connection:
        return await connection.get("userinfo.json")


@app.get("/api/v1/sprint/{dynamic}", response_model=SprintResponse)
